...
class Priors:
    ...
    def get_gp_cov_func(self, X, µ_vector, coef_matrix=None, noise=None, kernel="expquad", **kernel_params):
        ...
        if kernel == "linear" and coef_matrix is not None:
            ...
            return pm.gp.cov.RBF(1, coef_matrix.shape[1]).full(X.T) # Compute the covariance matrix with a radial basis function kernel
        elif kernel == "matern":
            return pm.gp.cov.Matern52(1, coef_matrix.shape[1]).full(X.T)
        elif kernel == "poly":
            tau = 0.1
            cov_func = tau * pm.gp.cov.Polynomial(input_dim=len(X.T), **{k: v for k, v in kernel_params.items() if k in ["c", "d", "offset"]})
            return cov_func